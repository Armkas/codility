...
https://app.codility.com/demo/results/trainingY4CVCW-QGY/
...
public func solution(_ N : Int) -> Int {
    var index = 0
    while Int(pow(Double(2), Double(index))) <= N {//等于时候能整除 等于1
        index += 1
    }
    var save = 0
    for i in 0..<index {// index - 1还不超过N index 就超过N
        if N % Int(pow(Double(2), Double(i))) == 0 {
            save = max(save, i)
        }
    }
    return save
